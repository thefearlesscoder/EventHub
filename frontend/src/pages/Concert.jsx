...
const Concert = () => {
    ...
    const [ concertdetails , setconcdetails ] = useState("") ;
      ...
      const searchConcert = async () => {
        try {
          const response = await fetch(`http://localhost:5000/api/v1/concert/concert/${id}`, {
            method: "POST", 
            headers: {
    ...
    const addingDetails = async () => {
        try {
            ...
            const response =  await fetch ( `http://localhost:5000/api/v1/concert/register-for-concert/${id}` , {
                method: "POST", 
                headers: {
                "Content-Type": "application/json",
                },
    ...
    const makePayment = async () => {
            ...
            try {
                const stripe = await loadStripe("pk_test_51QJ5RTAI8xVNoO7TqaukjHHfkOi5Nj0OPYYTToUwQkjukxrZ3RH0QZ92gH1bvqyUlxevQAz0hIHqkSomC1FFrPtQ00CCVZnGM8");
                ...
                const response = await fetch("http://localhost:5000/api/v1/concert/create-checkout-session", {
                    method: "POST",
                    headers: headers,
                    ...
                    md:flex-row flex-col md:text-auto text-center'>
                        ...
                        <div className=' flex justify-around mt-14 w-full md:flex-row flex-col gap-5 items-center '>
                            <div className=' font-bold text-4xl flex justify-center flex-col items-center md:text-auto text-center'>
                                <div>
                                    AvailableSeat - <span className='font-normal'>{concertdetails.seatingCapacity}</span>
                                </div>
                                <div className='font-bold flex gap-2 items-center md:text-auto text-center'>
                                    Price <div className='font-normal flex gap-2'> - <FaRupeeSign /> {concertdetails.ticketPrice}</div>
                                </div>
                            </div>
                            <div className={` w-[40px] h-[40px] 
                    rounded-full aspect-auto ${concertdetails.seatingCapacity > 0 ? "bg-caribbeangreen-600" : "bg-pink-400"} `}>
